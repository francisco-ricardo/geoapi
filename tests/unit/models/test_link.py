...
class TestLinkModelQueries:
    ...
    def test_link_filtering_by_multiple_criteria(self, test_db_simple):
        ...
        links = [
            SimplifiedLink(link_id=1, road_name="Highway 1", road_type="highway", speed_limit=65),
            SimplifiedLink(link_id=2, road_name="Main Street", road_type="local", speed_limit=35),
            SimplifiedLink(link_id=3, road_name="Highway 2", road_type="highway", speed_limit=55),
            SimplifiedLink(link_id=4, road_name="Oak Street", road_type="local", speed_limit=30),
            SimplifiedLink(link_id=5, road_name="Rural Road", road_type="rural", speed_limit=45)
        ]
        ...
        highway_links = test_db_simple.query(SimplifiedLink).filter(
            SimplifiedLink.road_type == "highway",
            SimplifiedLink.speed_limit > 60
        ).all()
        ...
        street_links = test_db_simple.query(SimplifiedLink).filter(
            SimplifiedLink.road_name.like("%Street%")
        ).all()
    ...
    def test_link_ordering_and_pagination(self, test_db_simple):
        ...
        ordered_links = test_db_simple.query(SimplifiedLink).order_by(
            SimplifiedLink.length.desc()
        ).all()
        ...
        page_2 = test_db_simple.query(SimplifiedLink).order_by(
            SimplifiedLink.link_id
    ...
    def test_link_aggregations(self, test_db_simple):
        ...
        links = [
            SimplifiedLink(link_id=1, road_name="Road 1", length=1000.0, road_type="highway"),
            SimplifiedLink(link_id=2, road_name="Road 2", length=2000.0, road_type="highway"),
            SimplifiedLink(link_id=3, road_name="Road 3", length=1500.0, road_type="local"),
            SimplifiedLink(link_id=4, road_name="Road 4", length=500.0, road_type="local"),
        ]
        ...
        highway_avg = test_db_simple.query(func.avg(SimplifiedLink.length)).filter(
            SimplifiedLink.road_type == "highway"
        ).scalar()
        assert highway_avg == 1500.0
        
        # Test count by road type
        local_count = test_db_simple.query(func.count(SimplifiedLink.link_id)).filter(
            SimplifiedLink.road_type == "local"
        ).scalar()
...
class TestLinkModelRelationships:
    ...
    def test_link_cascade_operations(self, test_db_simple):
        ...
        assert test_db_simple.query(SimplifiedLink).filter(
            SimplifiedLink.link_id == 1
        ).first() is None
...
class TestLinkModelValidation:
    ...
    def test_link_required_fields(self, test_db_simple):
        ...
        stored_link = test_db_simple.query(SimplifiedLink).filter(
            SimplifiedLink.link_id == 1
        ).first()
...
class TestLinkModelEdgeCases:
    ...
    def test_link_query_with_no_results(self, test_db_simple):
        ...
        result = test_db_simple.query(SimplifiedLink).filter(
            SimplifiedLink.link_id == 999
        ).first()
        
        assert result is None
        
        # Query with impossible conditions
        results = test_db_simple.query(SimplifiedLink).filter(
            SimplifiedLink.length < 0
        ).all()
