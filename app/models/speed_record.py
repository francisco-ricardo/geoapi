...
class SpeedRecord(Base):
    ...
    id = Column(
        Integer, primary_key=True, index=True, comment="Auto-generated primary key"
    )
    ...
    # Speed measurement
    speed = Column(Float, nullable=False, comment="Speed measurement in mph")

    # Derived temporal attributes for efficient querying
    day_of_week = Column(
        String, nullable=True, index=True, comment="Day of week (Monday, Tuesday, etc.)"
    )
