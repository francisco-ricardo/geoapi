    ...
    for link in links:
        ...
        if link.geometry is not None:
            try:
                ...
                logger.error(f"Error converting geometry to GeoJSON for link_id={link.link_id}", extra={
                    "error": str(e),
...
async def get_link(
    ...
    if link.geometry is not None:
        try:
            ...
            logger.error(f"Error converting geometry to GeoJSON for link_id={link_id}", extra={
                "error": str(e),
                "link_id": link_id
            })
            geometry_dict = None
...
async def create_link(
    ...
    if link.geometry:
        try:
            ...
            logger.error(f"Error converting GeoJSON to PostGIS geometry", extra={
                "error": str(e),
                "link_id": link.link_id
            })
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Database integrity error creating link", extra={
            "error": str(e),
            "link_id": link.link_id
        })
        raise HTTPException(
        ...
        db.rollback()
        logger.exception(f"Internal server error creating link", extra={
            "error": str(e),
            "link_id": link.link_id
        })
        raise HTTPException(
