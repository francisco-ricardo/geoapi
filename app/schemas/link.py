...
class LinkBase(BaseModel):
    ...
    length: Optional[float] = Field(
        default=None, ge=0, description="Road length in meters", examples=[1250.5]
    )
    ...
    speed_limit: Optional[int] = Field(
        default=None, ge=0, le=200, description="Speed limit in mph", examples=[35]
    )
...
class LinkCreate(LinkBase):
    ...
    link_id: int = Field(
        description="Unique identifier for the road link", examples=[12345]
    )
    geometry: Optional[Dict[str, Any]] = Field(
        default=None, description="Road segment geometry as GeoJSON LineString in WGS84"
    )
...
class LinkResponse(LinkBase):
    ...
    speed_records_count: Optional[int] = Field(
        default=None, description="Number of speed records for this link"
    )
...
class LinkList(BaseModel):
    ...
    total: int = Field(ge=0, description="Total number of links")
