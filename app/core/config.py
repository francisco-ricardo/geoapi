...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="", description="PostgreSQL database URL with PostGIS support"
    )
    ...
    tracing_provider: Optional[Literal["otlp", "jaeger", "honeycomb"]] = Field(
        default=None, description="Tracing provider to use"
    )
    tracing_endpoint: Optional[str] = Field(
        default=None, description="Endpoint for tracing provider"
    )

    # Mapbox settings (optional for API, required for notebooks)
    mapbox_access_token: Optional[str] = Field(
        default=None, description="Mapbox access token for visualization"
    )
