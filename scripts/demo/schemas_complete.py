...
def demo_geospatial_features():
    ...
    if link_with_geometry.geometry:
        ...
        print(
            f"   Coordinates: {len(link_with_geometry.geometry['coordinates'])} points"
        )
...
def demo_speed_records():
    ...
    print(f"   Speed: {speed_record.speed} mph")
    print(f"   Timestamp: {speed_record.timestamp}")
    print(f"   Day of week: {speed_record.day_of_week}")
    print(f"   Time period: {speed_record.time_period}")
    ...
    try:
        ...
        SpeedRecordCreate(
            link_id=1, timestamp=datetime.now(), speed=-10.0  # Invalid negative speed
        )
    except ValidationError as e:
        print(f"   ValidationError: {e.errors()[0]['msg']}")
    ...
    try:
        ...
        SpeedRecordCreate(
            link_id=1, timestamp=datetime.now(), speed=500.0  # Unrealistic speed
        )
    except ValidationError as e:
        print(f"   ValidationError: {e.errors()[0]['msg']}")
...
2. SPEED RECORD Management:
   - SpeedRecordBase: Core speed measurement fields (timestamp, speed, day_of_week, time_period)
...
3. VALIDATION Features:
   ...
   - Required field enforcement (link_id, timestamp, speed)
