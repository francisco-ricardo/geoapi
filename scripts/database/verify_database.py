...
def check_database_state():
    ...
    with Session() as session:
            ...
            if sample_link:
                ...
                try:
                    ...
                    result = session.execute(
                        text(
                            "SELECT ST_AsText(geometry) as geom_text FROM links WHERE link_id = :link_id"
                        ),
                    ...
                    if result and result.geom_text:
                        ...
                        print(
                            f"  Sample geometry (first 100 chars): {geom_text[:100]}..."
                        )
                    else:
        ...
        if speed_count > 0:
            ...
            if sample_speed:
                print(
                    f"  Sample speed record: Link {sample_speed.link_id}, {sample_speed.speed} mph"
                )
        ...
        try:
            ...
            result = session.execute(
                text("SELECT COUNT(*) FROM links WHERE ST_IsValid(geometry)")
            ).fetchone()
            if result:
