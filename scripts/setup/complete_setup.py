...
def start_containers():
    ...
    result = run_command("docker compose -f docker-compose-dev.yml up -d db", 
...
def wait_for_database():
    ...
    while attempt < max_attempts:
        ...
        result = run_command(
            ...
            "Check database readiness", check_output=True
        )
...
def verify_database_setup():
    ...
    result = run_command(
        ...
        "Check database tables", check_output=True
    )
    
    if result and isinstance(result, str) and "links" in result and "speed_records" in result:
        ...
        result = run_command(
            ...
            "Check sample data", check_output=True
        )
        
        if result and isinstance(result, str) and any(char.isdigit() for char in result):
...
def start_api():
    ...
    result = run_command("docker compose -f docker-compose-dev.yml up -d api", 
                        "Start API container")
...
def main():
    ...
    if not Path("/workspace/app").exists() or not Path("/workspace/docker-compose-dev.yml").exists():
    ...
    for step_name, step_func in steps:
        if not step_func():
            ...
            print("2. Check container logs: docker compose -f docker-compose-dev.yml logs")
            print("3. Reset containers: docker compose -f docker-compose-dev.yml down -v")
            sys.exit(1)
