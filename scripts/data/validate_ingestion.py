...
def main():
    ...
    print("Validating data integrity between original Parquet files and PostgreSQL database")
...
def load_sample_parquet_data(sample_size: int = 100) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def validate_link_data(link_sample: pd.DataFrame) -> bool:
    ...
    with Session() as session:
        for idx, parquet_row in link_sample.iterrows():
            ...
            if not db_link:
                all_passed = print_result(False, f"Link {link_id} not found in database")
                continue
            
            # Validate basic fields
            expected_road_name = parquet_row['road_name'] if pd.notna(parquet_row['road_name']) else None
            ...
            if expected_length is not None and actual_length is not None:
                if abs(expected_length - actual_length) > 0.0001:  # Tolerance for float precision
                    all_passed = print_result(False,
...
def validate_link_geometry(session, link_id: int, original_geojson: str) -> bool:
    ...
    try:
        ...
        if original_geom.geom_type == 'MultiLineString':
            if isinstance(original_geom, MultiLineString) and len(original_geom.geoms) > 0:
        ...
        result = session.execute(
            text("SELECT ST_AsGeoJSON(geometry, 6) as geom_json FROM links WHERE link_id = :link_id"),
            {"link_id": link_id}
        ).fetchone()
...
def validate_speed_data(speed_sample: pd.DataFrame) -> bool:
    ...
    with Session() as session:
        for idx, parquet_row in speed_sample.iterrows():
            ...
            db_speed = session.query(SpeedRecord).filter(
            ...
            print_result(True, f"Speed record for link {link_id} at {timestamp} validation passed")
...
def validate_data_integrity() -> bool:
    ...
    with Session() as session:
        ...
        orphaned_speeds = session.execute(text("""
        ...
        invalid_geometries = session.execute(text("""
            SELECT COUNT(*) as count
            FROM links
        ...
        if invalid_geometries.count > 0:
            all_passed = print_result(False,
                f"Found {invalid_geometries.count} links with invalid geometries")
        else:
            print_result(True, "All links have valid geometries")
        
        # Check coordinate system consistency
        srid_check = session.execute(text("""
        ...
        if len(srid_check) != 1 or srid_check[0].srid != 4326:
            ...
            print_result(True, f"All geometries use consistent SRID: {srid_check[0].srid}")
...
def validate_statistical_consistency() -> bool:
    ...
    with Session() as session:
        ...
        period_mapping = {1: 'Overnight', 2: 'Early Morning', 3: 'AM Peak', 
                         4: 'Midday', 5: 'Early Afternoon', 6: 'PM Peak', 7: 'Evening'}
        
        for period_id, period_name in period_mapping.items():
            parquet_avg = speed_df[speed_df['period'] == period_id]['average_speed'].mean()
            
            db_avg_result = session.execute(text("""
                SELECT AVG(speed) as avg_speed
                FROM speed_records
                WHERE time_period = :period_name
            """), {"period_name": period_name}).fetchone()
            ...
            if abs(parquet_avg - db_avg) > 0.1:  # 0.1 mph tolerance
                ...
                print_result(True, f"Average speed for {period_name} matches: {db_avg:.2f} mph")
