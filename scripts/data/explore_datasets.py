...
def explore_dataset(file_path, dataset_name):
    ...
    try:
        ...
        for i, col in enumerate(df.columns, 1):
            ...
            print(f"  {i:2d}. {col:<25} | {str(dtype):<15} | {null_count:>6} nulls ({null_pct:5.1f}%)")
...
def main():
    ...
    if link_df is not None and speed_df is not None:
        ...
        geometry_cols = [col for col in link_df.columns if 'geometry' in col.lower() or 'geom' in col.lower() or 'wkt' in col.lower()]
        if geometry_cols:
            print(f"\nGeometry columns found: {geometry_cols}")
            for col in geometry_cols:
                sample_geom = link_df[col].dropna().iloc[0] if not link_df[col].dropna().empty else None
                if sample_geom:
